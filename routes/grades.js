...
// lerner_id and class_id
router.get("/learner/:learner_id/class/:class_id", async (req, res) => {
  const collection = await db.collection("grades");
  const query = {learner_id: Number(req.params.learner_id),
     class_id: Number(req.param.class_id)
  };
...
// GET route weighted average score for each class for a learner
router.get("/learner/:learner_id/class/:class_id/", async (req, res) =>{
  const collection = await db.collection("grades");
